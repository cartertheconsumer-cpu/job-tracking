...
const INITIAL_JOBS = [
  ...
  { id:32, track:"Tech Sales", company:"Motive", role:"Sales Development Representative", url:"https://gomotive.com/careers/", location:"Remote-eligible", mode:"Remote", pay:"Competitive OTE · Strong SDR to AE track", commute:"Remote", repvue:"", summary:"Fleet management and IoT platform -- telematics, safety, spend management for physical economy businesses. Growing aggressively, hires SDRs in classes with structured promotion to AE.", dateApplied:"", followUp:"", status:"Watchlist", notes:"Check careers page for current SDR class openings. Strong SDR-to-AE pipeline." },
  { id:38, track:"Tech Sales", company:"Nooks", role:"Sales Development Representative", url:"https://www.nooks.ai/careers", location:"Remote (US)", mode:"Remote", pay:"~$70K base / $115K OTE · Top: $180K-$210K", commute:"Remote", repvue:"", summary:"AI-powered outbound sales platform (parallel dialer + virtual salesfloor). SDRs use the product daily. ~84% quota attainment. 4.4/5 compensation rating on Glassdoor.", dateApplied:"", followUp:"", status:"Watchlist", notes:"HIGH OTE ceiling. They want 18mo+ SaaS SDR exp but your 6.5yr consultative sales + outbound volume at UWM is a strong case." },
  { id:39, track:"Tech Sales", company:"LegalOn Technologies", role:"Sales Development Representative", url:"https://legalontech-inc.rippling-ats.com/", location:"Remote (US)", mode:"Remote", pay:"Competitive base + commission", commute:"Remote", repvue:"", summary:"Leader in AI contract review software for legal teams. SDRs identify potential customers and generate pipeline. AI-native product, high-growth. Remote-first.", dateApplied:"", followUp:"", status:"Watchlist", notes:"AI contract review SaaS. Your multi-party process experience (closings, compliance) maps well to selling to legal teams." },
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await window.storage.get("jst_jobs_v5");
        if (r?.value) { setJobs(JSON.parse(r.value)); }
        else { setJobs(INITIAL_JOBS); await window.storage.set("jst_jobs_v5", JSON.stringify(INITIAL_JOBS)); }
      } catch { setJobs(INITIAL_JOBS); }
      setLoaded(true);
    })();
  }, []);

  const save = async (updated) => { setJobs(updated); try { await window.storage.set("jst_jobs_v5", JSON.stringify(updated)); } catch {} };
